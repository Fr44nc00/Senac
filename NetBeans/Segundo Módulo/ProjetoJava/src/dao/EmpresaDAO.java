...
public class EmpresaDAO {
    ...
    public List<Empresa> getEmpresa(String nomeempresa ) { //parâmetro para buscar a empresa 
        //pelo nome
        String sql = "SELECT * FROM empresa WHERE nomeempresa LIKE ?"; //LIKE nos permite 
        //pesquisar por partes de um nome, ao invés de pesquisarmos por todo nome

        try {
            PreparedStatement stmt = this.conn.prepareStatement(sql);
            //Como temos um parâmetro, devemos defini-lo
            stmt.setString(1,"%" + nomeempresa + "%"); //Conforme for a palavra ou letra
            ...
            //Se o método entrar no "Catch" quer dizer que não encontrou nenhuma empresa,
